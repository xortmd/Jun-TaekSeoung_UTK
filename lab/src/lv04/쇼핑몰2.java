	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			// 아이템 관리
				...
				// 아이템 관리
				for(int i = 0; i < items.length; i++) {
					if(categoryIndex > 0 && items[categoryIndex - 1][0].equals(items[i][0])
							&& items[i][0] != "" && 100 < categoryIndex) {
						...
						// 아이템 삭제
							...
							System.out.print("삭제할 아이템을 입력해주세요. ");
							String item = scan.next();
							if(items[i][1].contains(item)) {
								items[i][1] = items[i][1].replace(item + "/", "");
								System.out.println(items[i][0] + "에서 "+ item + "이(가) 삭제되었습니다.");
							} else {
